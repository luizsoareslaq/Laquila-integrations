...
namespace Laquila.Integrations.Core.Infra.Interfaces
{
    public interface IViewsRepository
    {
        ...
        Task<(IEnumerable<VMWMS_BuscarNotasNaoIntegradas>, int totalCount)> GetNotasOutboundAsync(List<(string loMaCnpjOwner, long oeErpOrder)>? prenotas, int pageSize = 10);
        ...
        Task<(IEnumerable<VMWMS_BuscarRecebimentosNaoIntegrados>, int totalCount)> GetReceiveInvoicesAsync(int pageSize = 10);
    }
}